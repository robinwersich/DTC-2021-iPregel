---
title: "DTC 2021 - Analysis Betweenness Centrality vs. h-index"
...
bc <- as.data.frame(read.csv("../results/networkit_betweenness_centrality_author-citation_reversed.txt", sep="\t", header=FALSE))
colnames(bc) <- c("ID", "BC")
...
```{r}
plot( bc$BC, h_index$h.index)
plot( bc$BC, h_index$h.index, log="x")
```

3a. Pearson Correlation
```{r}
cor.r <- cor(bc$BC, h_index$h.index)
cor.r2 <- cor.r ^ 2
```
...
```{r}
spearman.cor.r <- cor(h_index$h.index, bc$BC, method="spearman")
