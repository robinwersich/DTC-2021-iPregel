---
title: "Template Network Analysis"
...
```{r}
pr = read.csv("../results/PAGERANK_PATH.txt", sep="\t", header = FALSE)
colnames(pr) <- c("ID", "PageRank")

bc = read.csv("../results/BETWEENNESS_CENTRALITY_PATH.txt", sep="\t", header = FALSE)
colnames(bc) <- c("ID", "BetweennessCentrality")

cc = read.csv("../results/CLOSENESS_CENTRALITY_PATH.txt", sep="\t", header = FALSE)
colnames(cc) <- c("ID", "ClosenessCentrality")

new_measures <- list("PageRank"=pr,"BetweennessCentrality"=bc,"ClosenessCentrality"=cc)

classical_measure = read.csv("../results/CLASSICAL_MEASURE_PATH.txt", sep="\t")
colnames(h_index) <- c("ID", "Name", "CLASSICAL MEASURE")

# transform ids to be 1-indexed to be able to work easily with them in R
bc$ID <- bc$ID + 1
cc$ID <- cc$ID + 1
pr$ID <- pr$ID + 1
classical_measure$ID <- classical_measure$ID + 1
