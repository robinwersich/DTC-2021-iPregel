---
title: "DTC 2021 - Analysis Closeness Centrality vs. follower count"
...
cc <- as.data.frame(read.csv("../results/networkit_closeness_centrality_higgs-activity_time_reversed.txt", sep="\t", header=FALSE))
colnames(cc) <- c("ID", "CC")
cc$ID <- cc$ID + 1


f_count = as.data.frame(read.csv("../results/follower_count_higgs-social_network.txt", sep=" ", header=FALSE))
colnames(f_count) <- c("ID", "f_count")
f_count <- f_count[1:456623,]
f_count.normalized <- f_count
f_count.sum <- sum(f_count["f_count"])
f_count.normalized["f_count"] <- apply(f_count.normalized["f_count"], 2, function(x) {x / f_count.sum})
```
...
```{r}
data.combined <- cbind(cc, f_count)
data.combined
```
3. How correlated are they?

```{r}
plot(top.vertices.h_index$h.index, top.vertices.cc$CC)
```
```{r}
...
```{r}
plot(f_count$f_count, cc$CC)
```

3a. Pearson Correlation
```{r}
cor.r <- cor(cc$CC, f_count$f_count)
cor.r2 <- cor.r ^ 2
```
...
```{r}
spearman.cor.r <- cor(f_count$f_count, cc$CC, method="spearman")
