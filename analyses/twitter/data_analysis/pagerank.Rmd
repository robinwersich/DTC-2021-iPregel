---
title: "DTC 2021 - Analysis Pagerank vs. follower count"
...
pr <- as.data.frame(read.csv("../results/networkit_pagerank_higgs-activity_time.txt", sep="\t", header=FALSE))
colnames(pr) <- c("ID", "PR")
pr$ID <- pr$ID + 1
...
```{r}
data.combined <- cbind(pr, f_count)
data.combined
```
...
```{r}
plot(f_count$f_count, pr$PR)
```

3a. Pearson Correlation
```{r}
cor.r <- cor(f_count$f_count, pr$PR)
cor.r2 <- cor.r ^ 2
```
