...
names(performance) = c(
  "cc - Citation", "cc - Twitter", "cc - IMDb", "cc - stackoverflow",
...
Let's compare the runtimes for loading, calculating and dumping in a table for min and max thread count:
...
for (time.type in c("load", "calc", "dump")) {
  time.df = do.call("rbind", lapply(performance, function(df) {
    ...
    for (thread.count in c(1, 32)) {
      ...
      #df.new[[paste("diff", thread.count, sep = ".")]] = round(relative.diff, 3)
...
for (dataset in datasets) {
  ...
  barplot(t(memory.matrix), main=dataset, xlab = "max. memory consumption in MB",
          ...
          legend.text = FALSE, args.legend = list(x="bottomright"), col = unlist(frameworks.colors))
}
```
